...
filename = 'C:/Users/yanral/Documents/Software Development/Examples/'\
    'Example loading CellML model/tentusscher_2006_pints_and_Chons_hERG.mmt'
...
for i in range(len(fitting_param_annot)):
    sabs_pkpd.constants.s.set_constant(fitting_param_annot[i],
                                       value_generate_rescale[i])
...
sabs_pkpd.constants.data_exp = sabs_pkpd.load_data.Data_exp(
    times, values, exp_nums_list, exp_conds_list)
sabs_pkpd.constants.data_exp.Add_fitting_instructions(
    fitting_param_annot, exp_cond_annot, readout)


# Select a starting point close to the settings used for generating the
# synthetic data
...
for i in range(len(value_generate_rescale)):
    set_to_test[i] = np.random.uniform(low=0.9, high=1.11, size=None) \
        * value_generate_rescale[i]
starting_point = [np.array(list(set_to_test) + [Noise_sigma])]


# Run the MCMC routine calling the function in the sabs_pkpd package
chains = sabs_pkpd.pints_problem_def.\
    MCMC_inference_model_params(starting_point, max_iter=3000)
# Only one chain is run so this is the chain we select for analysis
chain_index = 0


# Plot a 2D map of distribution of parameter i against parameter j
fig, ax = plt.subplots(1, 1)
sabs_pkpd.pints_problem_def.plot_kde_2d(0,
                                        1,
                                        chains,
                                        ax,
...
sabs_pkpd.pints_problem_def.plot_distribution_map(
    mcmc_chains=chains,
    RealValue=value_generate_rescale,
    chain_index=chain_index,
    fig_size=(25, 25))

# Plot comparison between synthetic data, the model output for the starting
# point, and the endpoint of the chain

plt.figure()
for i in range(len(sabs_pkpd.constants.data_exp.exp_conds)):
    sabs_pkpd.constants.s.set_constant(
        sabs_pkpd.constants.data_exp.fitting_instructions.exp_cond_param_annot,
        sabs_pkpd.constants.data_exp.exp_conds[i])
    time_samples = sabs_pkpd.constants.data_exp.times[i]

    starting_point = chains[chain_index][0, :]
    sabs_pkpd.constants.s.reset()
    for j in range(len(sabs_pkpd.constants.data_exp.
                       fitting_instructions.fitted_params_annot)):
        sabs_pkpd.constants.s.set_constant(
            sabs_pkpd.constants.data_exp.
            fitting_instructions.fitted_params_annot[j],
            starting_point[j])
    sim = sabs_pkpd.constants.s.run(time_samples[-1] * 1.001,
                                    log_times=time_samples)
