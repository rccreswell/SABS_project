...
def test_get_steady_state():
    ...
    expected1 = np.array([-85.4, 0.0001076, 3.65, 0.000224, 0.986, 10.11, 135.38, 0.652, 0.0231, 0.0033, 0.00167, 0.749, 0.748,
                3.312e-05, 0.967, 0.999, 0.99996, 0.999998, 2.363e-08])
    res = np.array(res[0][0])
    rel_diff = abs((expected1-res)/res)
    print(np.nanmax(rel_diff) < 0.01)
    ...
    expected2 = np.array([-87.93, 0.0129, 7.279, 7.279, 144.64, 144.64, 8.72e-05, 8.622e-05, 1.62, 1.562, 0.0074, 0.6956, 0.6956,
                 ...
                 3.118e-07])
    test = np.array(res2[0][0])
    rel_diff = abs((expected1 - test) / test)
    print(np.nanmax(rel_diff) < 0.01)
    test = np.array(res2[1][0])
    rel_diff = abs((expected2 - test) / test)
    print(np.nanmax(rel_diff) < 0.01)
