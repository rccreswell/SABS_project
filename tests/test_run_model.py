...
class Test(unittest.TestCase):
    def test_simulate_data(self):
        ...
        with self.assertRaises(ValueError) as context:
            out = sabs_pkpd.run_model.simulate_data(wrong_fitted_params_values, sabs_pkpd.constants.s, sabs_pkpd.constants.data_exp)
    ...
    def test_quick_simulate(self):
        ...
        test2 = sabs_pkpd.run_model.quick_simulate(s, time_max, 'comp1.y', time_samples=time_samples,
        ...
        with self.assertRaises(ValueError) as context:
            test = sabs_pkpd.run_model.quick_simulate(s, wrong_time_max, 'comp1.y', time_samples=wrong_time_samples)
        ...
        with self.assertRaises(ValueError) as context:
            test = sabs_pkpd.run_model.quick_simulate(s, time_max, 'comp1.y', time_samples=time_samples,
                                                   fixed_params_annot=changed_params_names,
                                                   fixed_params_values=changed_params_values)
        ...
        with self.assertRaises(ValueError) as context:
            test = sabs_pkpd.run_model.quick_simulate(s,time_max, 'comp1.y', time_samples=time_samples,
                                                   ...
                                                   exp_cond_param_values=[37])
        assert 'parameter annotation must be a string' in str(context.exception)
