...
class Test(unittest.TestCase):
    ...
    def test_compute_APD(self):
        ...
        AP = sabs_pkpd.run_model.quick_simulate(
        ...
        times = np.hstack((np.linspace(0, 250, 1001),
                           np.linspace(251, 1000, 250)))
    ...
    def test_compute_calcium_transient_duration(self):
        mmt = './tests/test resources/'\
              'tentusscher_2006_pints_and_Chons_hERG.mmt'
        sabs_pkpd.constants.s = \
            sabs_pkpd.load_model.load_simulation_from_mmt(mmt)

        Cai = sabs_pkpd.run_model.quick_simulate(
            sabs_pkpd.constants.s,
            1000,
            'calcium.Ca_i',
            pre_run=150000,
        ...
        expected_CaiD90 = 418
        CaiD50 = sabs_pkpd.cardiac.\
            compute_calcium_transient_duration(Cai, repol_percentage=50)
        expected_CaiD50 = 168
        ...
        Cai = sabs_pkpd.run_model.quick_simulate(sabs_pkpd.constants.s,
                                                 ...
                                                 time_samples=times)
        Cai = Cai[0]
        CaiD90 = sabs_pkpd.cardiac.\
            compute_calcium_transient_duration(Cai, time_points=times)
        expected_CaiD90 = 382
        assert (np.isclose(CaiD90, expected_CaiD90, atol=2))

        CaiD90 = sabs_pkpd.cardiac.\
            compute_calcium_transient_duration(Cai,
                                               time_points=times,
                                               upstroke_time=50)
        ...
        # Test exceptions
        with self.assertRaises(ValueError) as context:
            CaiD = sabs_pkpd.cardiac.\
                compute_calcium_transient_duration('not_cai')
        assert 'Cai should be provided' in str(context.exception)

        with self.assertRaises(ValueError) as context:
            CaiD = sabs_pkpd.cardiac.\
                compute_calcium_transient_duration(
                    Cai, time_points='not time points')
        assert 'time_points should be provided' in str(context.exception)

        # Test upstroke time not in time_points
        CaiD90 = sabs_pkpd.cardiac.\
            compute_calcium_transient_duration(
                Cai, time_points=times, upstroke_time=50.0001)
        expected_CaiD90 = 389.5
        assert (np.isclose(CaiD90, expected_CaiD90, atol=2))
