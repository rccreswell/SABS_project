...
class Test(unittest.TestCase):
    def test_infer_params(self):
        ...
        with self.assertRaises(ValueError) as context:
            inferred_params, found_value = sabs_pkpd.pints_problem_def.infer_params(wrong_initial_point, sabs_pkpd.constants.data_exp, boundaries_low, boundaries_high)
        assert 'should have the same length' in str(context.exception)

        wrong_boundaries_low = [0.0]
        with self.assertRaises(ValueError) as context:
            inferred_params, found_value = sabs_pkpd.pints_problem_def.infer_params(initial_point, sabs_pkpd.constants.data_exp, wrong_boundaries_low, boundaries_high)
        assert 'should have the same length' in str(context.exception)

        wrong_boundaries_high = [1.0]
        with self.assertRaises(ValueError) as context:
            inferred_params, found_value = sabs_pkpd.pints_problem_def.infer_params(initial_point, sabs_pkpd.constants.data_exp, boundaries_low, wrong_boundaries_high)
        assert 'should have the same length' in str(context.exception)
    ...
    def test_MCMC_routine(self):
        ...
        with self.assertRaises(ValueError) as context:
            chains = sabs_pkpd.pints_problem_def.MCMC_routine(wrong_starting_point, max_iter=10)
        assert 'must have the same length' in str(context.exception)

        # test loading from mmt
        chains = sabs_pkpd.pints_problem_def.MCMC_routine(starting_point, max_iter=10, mmt_model_filename='./tests/test resources/tentusscher_2006_pints_and_Chons_hERG.mmt')
