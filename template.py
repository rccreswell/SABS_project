...
# Data is provided in 3 columns : Time in hours, Concentration in ng/mL, Dose in mg/kg
...
IV_deliv_time = 0.01 # in hours. Make sure it matches with the protocol of .mmt model

# Compute and convert dose(t) in ng
total_drug = [IV_dose[i]*1e6*mouse_weight*1e-3 for i in range(len(IV_dose))]
...
class MyModel(pints.ForwardModel):
    def n_parameters(self):
        # Define the amount offitted parameters
        return 6
...
problem = pints.SingleOutputProblem(model = MyModel(), times = np.linspace(0,24,10), values = fit_values)
boundaries = pints.RectangularBoundaries([3, 5, 7, 5, 0.5, 30],[15, 15, 30, 20, 5, 60])
...
for i in range(len(dose)):  
    ...
    # times array is used. The times values have to be increasing, hence the +0.1
