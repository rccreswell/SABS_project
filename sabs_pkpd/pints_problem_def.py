...
def parameter_is_state(param_annot, myokit_simulation):
    """"
    Returns whether the variable param_annot is a state variable of the
    myokit.Simulation provided as argument
    ...
    :return is_state:
    bool. Returns True if the variable param_annot is a state variable, False
    otherwise.
    """
...
def find_index_of_state(param_annot, myokit_simulation):
    ...
    :return:
    int. Returns the index of the state variable. None if the variable could
    not be found.
    """
    index = None
    for i in range(len(myokit_simulation._model._state)):
        if myokit_simulation._model._state[i]._component._name + '.' \
                + myokit_simulation._model._state[i]._name == param_annot:
            index = i
...
def plot_distribution_parameters(mcmc_chains: list,
                                 bound_min: list,
                                 bound_max: list,
                                 chain_index: int = 0,
                                 fig_size=(15, 15),
                                 explor_iter: int = 1000):
    """
    :param mcmc_chains:
    list. The list containing the MCMC chains obtained after running the MCMC
    routine. chain[i] returns the i-th chain.

    :param bound_min:
    list. List of length the amount of parameters sampled during the MCMC
    routine. bound_min[i] is the minimum boundary for the plot of the i-th
    parameter.

    :param bound_max:
    list. List of length the amount of parameters sampled during the MCMC
    routine. bound_max[i] is the maximum boundary for the plot of the i-th
    parameter.
    ...
    :param fig_size:
    tuple. Defines the size of the figure on which the distribution of
    parameters is plotted. (15, 15) if not specified by the user.

    :param explor_iter:
    int. Length of the exploratory phase, which is excluded when plotting the
...
def hist_1d(x, ax):
    ...
    ax.hist(x, bins=x2, density=True)
    kernel = stats.gaussian_kde(x)
    f = kernel(x1)
    ax.plot(x1, f)


def plot_MCMC_convergence(mcmc_chains,
                          expected_values,
                          bound_max,
                          bound_min,
                          parameters_annotations=None):
    """
    Plots the convergence of the MCMC chains, with boundaries and expected
    values.
    ...
    list. List containing the expected values of all of the parameters fitted
    during MCMC.
    ...
    List of strings. Names of the model parameters fitted (with noise being the
    last parameter) during MCMC
    ...
    if len(bound_min) != len(bound_max) or len(bound_min) != n_params:
        raise ValueError('Boundaries length must match the amount of '
                         'parameters. Length of low boundaries: ' +
                         str(len(bound_min)) + ', Length of upper boundaries: '
                         + str(len(bound_max)))

    if n_params != len(expected_values):
        raise ValueError('The expected values must have the same length as '
                         'the MCMC parameters.' +
    ...
    for i in range(n_params):
        if n_params > 2:
            ...
            for j in range(len(mcmc_chains)):
                axes[row, col].plot(mcmc_chains[j, :, i],
                                    label='chain ' + str(j),
                                    LineWidth=1.5)
            axes[row, col].legend()
